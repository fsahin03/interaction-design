const questions = {
    default: [{
        vraag: `No subject selected`,
        antwoord_a: `No subject selected`,
        antwoord_b: `No subject selected`,
        antwoord_c: `No subject selected`,
        antwoord_d: `No subject selected`
    }], 
    programeren: [ 
        {
...
// GEGEVENS DIE HIERBIJ HOREN const dutch = document.querySelector(`.dutch`);
 const dutch = document.querySelector('.dutch'); 
 const english = document.querySelector(`.english`);
 const math = document.querySelector(`.math`);
 const programmeerTaal = document.querySelector(`.programeren`);
 ...
 let totalPoints = 0; 
 let useCategory = 'default';
 let useQuestions = questions[useCategory];
console.log(useQuestions);
 dutch.addEventListener(`click`, function(){
    console.log(useQuestions);

     useCategory = 'NL';
     useQuestions = questions[useCategory];
     currentQuestion = 0;
     showQuestion(useQuestions[0]);
 });
...
for (let i = 0; i < answerButtons.length; i++) {
    const theBtn = answerButtons[i];
        ...
        if (theBtn.classList.contains(correctAnswer)) {
            ...
            showQuestion(useQuestions[currentQuestion]);
        } else {
            currentQuestion++;
            alert(`wrong Answer`)
            showQuestion(useQuestions[currentQuestion]);
